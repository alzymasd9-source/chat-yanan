...
db.serialize(()=>{
  ...
  db.run("CREATE TABLE IF NOT EXISTS reports (id INTEGER PRIMARY KEY, reporter TEXT, reported TEXT, room TEXT, msg_id INTEGER, reason TEXT, status TEXT)");
});
...
io.on('connection', (socket)=>{
  ...
  socket.on('disconnect',()=>{
    for(let n in onlineUsers){ if(onlineUsers[n]==socket.id) delete onlineUsers[n]; }
  });
});
...
app.post('/upload-private', upload.single('file'), (req,res)=>{
  ...
  db.run("INSERT INTO private_messages (room,from_name,to_name,content,type,filename,time) VALUES (?,?,?,?,?,?,?)",
    [room,sender,receiver,'/uploads/'+req.file.filename,type,req.file.originalname,time]);
