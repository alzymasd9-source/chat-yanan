...
app.post('/upload-avatar', upload.single('file'), (req, res) => {
  ...
  db.run("UPDATE members SET avatar = ? WHERE id = ?", [fileUrl, userId], (err) => {
    if (err) return res.status(500).send('Error');
    res.json({ url: fileUrl });
  });
});

app.post('/upload-cover', upload.single('file'), (req, res) => {
  const { userId } = req.body;
  if (!userId || !req.file) return res.status(400).send('Missing data');

  const fileUrl = '/uploads/' + req.file.filename;
  db.run("UPDATE members SET cover = ? WHERE id = ?", [fileUrl, userId], (err) => {
    if (err) return res.status(500).send('Error');
    res.json({ url: fileUrl });
  });
});
...
app.post('/upload-private', upload.single('file'), (req, res) => {
  ...
  if (!room || !sender || !receiver || !req.file) return res.status(400).send('Missing data');
  ...
  db.run("INSERT INTO private_messages (room, from_name, to_name, content, type, filename, time) VALUES (?, ?, ?, ?, ?, ?, ?)",
    [room, sender, receiver, fileUrl, type, req.file.originalname, time],
    (err) => {
      if (err) return res.status(500).send('Error');
      io.to(room).emit('private_message', {
        from: sender,
